...
function sortSelectedAnswers(suspend_data) {
    ...
    //Remove old 'w' value if there is any
    string = RemoveOldWeightInfo(string);
...
function RemoveOldWeightInfo(string_array)
{
    if (string_array[0].lastIndexOf('w') != -1)
    {
        for (var i = 0; i < string_array.length; i++)
        {
            string_array[i] = string_array[i].substring(0, string_array[i].lastIndexOf('w'));
        }
    }
